...
func TestTrackerBlockContainer_RemoveBlocks(t *testing.T) {
	...
	t.Run("Remove blocks from 'from' to 'last' index, and update lastProcessedConfirmedBlock", func(t *testing.T) {
		...
		tbc.AddBlock(&ethgo.Block{Number: 1, Hash: ethgo.Hash{1}, ParentHash: ethgo.Hash{0}})
		tbc.AddBlock(&ethgo.Block{Number: 2, Hash: ethgo.Hash{2}, ParentHash: ethgo.Hash{1}})
		tbc.AddBlock(&ethgo.Block{Number: 3, Hash: ethgo.Hash{3}, ParentHash: ethgo.Hash{2}})
		tbc.AddBlock(&ethgo.Block{Number: 4, Hash: ethgo.Hash{4}, ParentHash: ethgo.Hash{3}})
		tbc.AddBlock(&ethgo.Block{Number: 5, Hash: ethgo.Hash{5}, ParentHash: ethgo.Hash{4}})
	...
	t.Run("Remove blocks from 'from' to 'last' index, where given already removed", func(t *testing.T) {
		...
		// Add some blocks to the container
		tbc.AddBlock(&ethgo.Block{Number: 11, Hash: ethgo.Hash{11}, ParentHash: ethgo.Hash{10}})
		tbc.AddBlock(&ethgo.Block{Number: 12, Hash: ethgo.Hash{12}, ParentHash: ethgo.Hash{11}})
		tbc.AddBlock(&ethgo.Block{Number: 13, Hash: ethgo.Hash{13}, ParentHash: ethgo.Hash{12}})
		tbc.AddBlock(&ethgo.Block{Number: 14, Hash: ethgo.Hash{14}, ParentHash: ethgo.Hash{13}})
		tbc.AddBlock(&ethgo.Block{Number: 15, Hash: ethgo.Hash{15}, ParentHash: ethgo.Hash{14}})
...
func TestTrackerBlockContainer_AddBlockAndLastCachedBlock(t *testing.T) {
	...
	for i := uint64(1); i <= uint64(numOfBlocks); i++ {
		tbc.AddBlock(&ethgo.Block{Number: i, Hash: ethgo.Hash{byte(i)}, ParentHash: ethgo.Hash{byte(i - 1)}})
